...
layout(set = 0, binding = 1) uniform BlurDir {
    bool isHorizontal;
} blurdir;
...
void main() {
    ...
    // actually not good to use if in frag shader
    if(blurdir.isHorizontal) {
        for(int i = 1; i < 5; ++i) {
        ...
        for(int i = 1; i < 5; ++i) {
            result += texture(bloomSetup, uv + vec2(0.0, offset.y * i)).rgb * weight[i];
            result += texture(bloomSetup, uv - vec2(0.0, offset.y * i)).rgb * weight[i];
        }
    }
